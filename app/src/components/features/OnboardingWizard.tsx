...
import { CheckCircle2, Droplets, Loader2, Rocket, Wallet } from "lucide-react";
...
export function OnboardingWizard() {
  ...
  const [busy, setBusy] = useState<"none" | "faucet" | "setup">("none");
  ...
  const onFaucet = async () => {
    setBusy("faucet");
    try {
      await dapp.requestAirdrop(Number(airdropSol));
    } finally {
      setBusy("none");
    }
  };
  ...
  return (
    <ActionShell
      ...
      subtitle="Wallet checks, local faucet helper, and one-click protocol setup wizard."
    ...
    >
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-cyan-300/25 bg-[#081224]/75 p-4">
          <p className="inline-flex items-center gap-2 text-sm font-medium text-cyan-100">
            <Droplets className="size-4" /> Faucet helper
          </p>
          <p className="mt-1 text-xs text-cyan-100/65">Works on local validator and devnet faucets.</p>
          <label className="mt-3 block text-xs text-cyan-100/70">
            SOL amount
            <input
              value={airdropSol}
              onChange={(e) => setAirdropSol(e.target.value)}
              className="mt-1 w-full rounded-lg border border-cyan-300/30 bg-black/25 px-3 py-2 text-sm text-white outline-none focus:border-cyan-300"
            />
          </label>
          <button
            onClick={() => void onFaucet()}
            disabled={!dapp.connected || busy !== "none"}
            className="mt-3 inline-flex items-center gap-2 rounded-lg border border-cyan-300/45 bg-cyan-300/15 px-4 py-2 text-sm text-cyan-100 disabled:opacity-50"
          >
            {busy === "faucet" ? <Loader2 className="size-4 animate-spin" /> : <Wallet className="size-4" />}
            Request SOL
          </button>
        </div>
